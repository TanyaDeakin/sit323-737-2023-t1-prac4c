...
const app = express(); 
const fs = require('fs');
const { passport, generateToken } = require('./authentication');
...
if (email === 'authuser@email.com' && password === 'auth@123') {
...
app.get("/add", passport.authenticate('jwt', { session: false }), (req, res) => {
  ...
  try{
    ...
    logger.info('An authorized addition operation requested on '+num1+' and '+num2);
...
app.get("/subtract", passport.authenticate('jwt', { session: false }), (req, res) => {
  ...
  try{
    const num1= parseFloat(req.query.num1);
    const num2= parseFloat(req.query.num2);
    logger.info('An authorized Subtract operation requested on '+num1+' and '+num2);
    const test = check(num1,num2,req,res); 
    if(test == true) {
...
app.get("/multiply", passport.authenticate('jwt', { session: false }), (req, res) => {
  ...
  try{
    const num1= parseFloat(req.query.num1);
    const num2= parseFloat(req.query.num2);
    logger.info('An authorized Multiply operation requested on '+num1+' and '+num2);
    const test = check(num1,num2,req,res);
    if(test == true) {
...
app.get("/divide", passport.authenticate('jwt', { session: false }), (req, res) => {
  ...
  try{
    const num1= parseFloat(req.query.num1);
    const num2= parseFloat(req.query.num2);
    logger.info('An authorized Divide operation requested on '+num1+' and '+num2);
    const test = check(num1,num2,req,res);
    if(test == true) {
